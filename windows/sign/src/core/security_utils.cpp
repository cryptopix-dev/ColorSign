...
void initialize_security_monitor(std::unique_ptr<SecurityMonitor> monitor) {
    if (!monitor) {
        global_security_monitor = std::make_unique<DefaultSecurityMonitor>();
    } else {
...
void SecureMemory::secure_wipe(void* ptr, size_t size) {
    ...
    // Pass 1: Set to all 1s
    p = static_cast<volatile uint8_t*>(ptr);
    for (size_t i = 0; i < original_size; ++i) {
        p[i] = 0xFF;
    }

    // Pass 2: Set to all 0s
    for (size_t i = 0; i < original_size; ++i) {
        p[i] = 0x00;
    }

    // Pass 3: Set to alternating pattern
    for (size_t i = 0; i < original_size; ++i) {
        p[i] = 0xAA;
    }

    // Pass 4: Final wipe to zeros
    for (size_t i = 0; i < original_size; ++i) {
        p[i] = 0x00;
    }
}
...
TimingProtection::TimingProtection(std::unique_ptr<SecurityMonitor> monitor)
    ...
    if (!monitor_) {
        monitor_ = std::make_unique<DefaultSecurityMonitor>();
    }
}
